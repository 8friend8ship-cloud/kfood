param(
  [string]$TargetTitle = 'WEBAPP_TEMPLATE_09',
  [string]$ExpectedSpreadsheetId = '1K4Bj0PnnLD-Wka8AlNmDprvfXFnsh9mCV3QXjgdbT2s',
  [string]$ExpectedDeploymentId = 'AKfycby5rynajzdtrWcVuwd1W8kT37DHfFYgwg6Bg7C5dlm222FxIYrl3J5l1UQSYNp1FZ8IQg'
)
...
function Get-ClaspRoot([string]$ProjectDir) {
  ...
  if (-not (Test-Path -LiteralPath $configPath)) {
    return [System.IO.Path]::GetFullPath($ProjectDir)
  }
  try {
    $config = Get-Content -Raw -LiteralPath $configPath | ConvertFrom-Json
  } catch {
    throw 'CLASP_CONFIG_PARSE_FAILED'
  }
...
if (-not (Get-Command clasp -ErrorAction SilentlyContinue)) {
  throw 'CLASP_COMMAND_NOT_FOUND_EXISTING_RUNNER_REQUIRED'
}

# Existing authorization only. Never start a login flow from this diagnostic.
$authText = (& clasp show-authorized-user --json 2>&1 | Out-String)
if ($LASTEXITCODE -ne 0) {
  $authText = (& clasp show-authorized-user 2>&1 | Out-String)
  if ($LASTEXITCODE -ne 0) {
    throw 'EXISTING_CLASP_AUTH_NOT_AVAILABLE_DO_NOT_LOGIN_OR_CREATE_PROJECT'
  }
}

# Recover exactly one script by the existing project title.
...
# Read-only source snapshot. No clasp push/deploy/create commands are present in this script.
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$snapshotDir = Join-Path $env:TEMP "kfood-bound-script-readonly-$stamp"
...
# Verify the script belongs to the expected existing deployment lineage.
$deploymentText = (& clasp list-deployments $scriptId 2>&1 | Out-String)
if ($LASTEXITCODE -ne 0) { throw 'CLASP_LIST_DEPLOYMENTS_FAILED' }
if ($ExpectedDeploymentId -and $deploymentText -notmatch [regex]::Escape($ExpectedDeploymentId)) {
  throw 'EXPECTED_DEPLOYMENT_ID_NOT_FOUND_ABORT_READONLY_RECOVERY'
}

$sourceRoot = Get-ClaspRoot $snapshotDir
$files = @(Get-ChildItem -LiteralPath $sourceRoot -Recurse -File | Where-Object {
  $_.Name -ne '.clasp.json' -and $_.Name -notmatch '^\.claspignore$'
...
$result = [ordered]@{
  ...
  next = 'Return this JSON to central evidence. Do not push/deploy until source snapshot is reviewed against PR6.'
}
