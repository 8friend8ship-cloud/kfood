...
const callFirebaseFunction = async <T, R>(functionName: string, payload: T, _mockResponse: R): Promise<R> => {
  if (!functions) {
    throw new Error(`BACKEND_UNAVAILABLE: '${functionName}' is disabled until the approved server is configured.`);
  }
  try {
